...
local onlinePointsEvent = GlobalEvent("GainItemPerHour")
    ...
    for _, player in pairs(players) do
        ...
        if ip ~= 0 and (not config.checkDuplicateIps or not checkIp[ip]) then
            ...
            if seconds >= 3600 then
                ...
                if item then
			player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Voce recebeu um conch shell do sorteio por estar online por uma hora.")
                end
                return true
            end
