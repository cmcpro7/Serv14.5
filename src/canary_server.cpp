...
#ifdef _WIN32
SetConsoleTitleA("Play Otserver");
#endif
}
...
int CanaryServer::run() {
	g_dispatcher().addEvent(
		[this] {
			try {
				...
				logger.info("Servidor protocolo: {}.{}{}", CLIENT_VERSION_UPPER, CLIENT_VERSION_LOWER, g_configManager().getBoolean(OLD_PROTOCOL) ? " e 10x permitido!" : "");
				...
				logger.info("Todos os modulos carregados, servidor iniciando...");
				...
				logger.error("O programa sera fechado apos pressionar a tecla Enter...");
	...
	if (loaderStatus == LoaderStatus::FAILED || !serviceManager.is_running()) {
		logger.error("Nenhum servico em execucao. O servidor NAO esta online!");
		shutdown();
		return EXIT_FAILURE;
	}

	logger.info("{} {}", g_configManager().getString(SERVER_NAME), "\033[38;2;0;255;0mservidor online!\033[0m");
	g_logger().setLevel(g_configManager().getString(LOGLEVEL));
logger.info("\033[38;2;0;136;255mVelocidade de Conexao Otimizada para melhor Desempenho\033[0m");
...
logger.info("\033[38;2;186;85;255mDesenvolvido por: Play Otserver\033[0m");
logger.info("\033[38;2;30;144;255mVisite nosso site para updates, suporte e recursos: https://playotserver.com.br\033[0m");
...
void CanaryServer::badAllocationHandler() {
	...
	g_logger().error("Falha na alocacao, servidor sem memoria, "
	                 "diminua o tamanho do seu mapa ou compile no modo 64 bits");
...
void CanaryServer::initializeDatabase() {
	logger.info("Estabelecendo conexao com o banco de dados... ");
	...
	if (!DatabaseManager::isDatabaseSetup()) {
		throw FailedToInitializeCanary(fmt::format(
			"O banco de dados especificado em {} esta vazio. Importe o schema.sql para seu banco de dados.",
	...
	g_logger().info("Conexao da database estabelecida");
}
...
void CanaryServer::loadModules() {
	...
	logger.debug("Inicializando o ambiente lua...");
